...
def get_google_credentials(logger):
    """
    Summary:
    ...
    if not creds or not creds.valid:
        logger.warning("Failed to obtain valid creddentials")
        ...
        if creds and creds.expired and creds.refresh_token:
            ...
            logger.info("Attempting to authorise application...")
            try:
                ...
                logger.error("Failed to authorize application:", e)
...
def start_mysql_service(sudopass, logger):
    """
    Summary:
    ...
    for process in psutil.process_iter():
        if process.name() == "mysqld":
            logger.info(f"MySQL process is already running.")
            return
...
def pool_connection(logger):
    ...
    """
    start_mysql_service(sudopass, logger)
    ...
    logger.info(f"MySQL connection pool created.")
    return pool


def get_cursor(pool, logger):
    """
    Summary:
    ...
    try:
        ...
        logger.info(f"Connected to {pool.pool_name} pool successfully\n")
    except:
        pool.add_connection()
        logger.warning(f"Added a new connection to {pool.pool_name} pool\n")
        connection = pool.get_connection()
        logger.info(f"Connected to {pool.pool_name} pool successfully")
